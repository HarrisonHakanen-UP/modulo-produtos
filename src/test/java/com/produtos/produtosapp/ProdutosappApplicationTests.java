...
public class ProdutosappApplicationTests {
	...
	public void ProdutoCreate() {
		...
		/*
		 * ProdutoRepositorio.save(new Produto(1, "teste", "1","azul", 20, 3, 800,
		 * 1000,"3566", "teste", "teste", 50, 15, "teste", "teste", "teste", null, null,
...
//		acaoRepository.save(acao);
		 ...
		 * 
		 * // criar produto.setProdutoId(67); produto.setNome("M");
		 * produto.setNumeroProduto("8"); produto.setBandeira(true);
		 * produto.setBandeiraAcabado(false); produto.setCor("v");
		 * produto.setNivelEstoque(6); produto.setPontoReordenar(2);
		 * produto.setCustoPadrao(5); produto.setPrecoVenda(5);
		 * produto.setTamanhoProduto("2"); produto.setUnidadeMedidaTamanho("teste");
		 * produto.setUnidadeMedidaPeso("teste"); produto.setPesoProduto(2);
		 ...
		 * fotos.setNomeImagemGrande("bike"); fotos.setNomeImagemPequena("bikemini");
		 * fotos.setPrincipal(true); fotos.setProduto(produto);
		 * 
		 ...
		 * revisao.setComentarios("discos retificados"); revisao.setDataRevisao(data);
		 * revisao.setEnderecoEmail("mecanico1@bike.com");
		 * revisao.setNomeRevisao("Reparo freios"); revisao.setProduto(produto);
		 * 
		 ...
		 * 
		 * 
		 * descricao.setDataCadastro(data); descricao.setDescricao("barulhos traseiro");
		 * 
		 * 
		 * cultura.setDataCadastro(data); cultura.setNome("para o barro");
		 * 
		 * 
		 * documentos.setAdministrador(12); documentos.setAlterarNumero(45);
		 ...
		 * documentos.setNomeArquivo("First 10"); documentos.setPastaDocumento(true);
		 * documentos.setProduto(produto);
		 * documentos.setResumoDocumento("correções parte transmissão");
		 * documentos.setRevisaoArquivo("final"); documentos.setStatus(10);
		 * documentos.setTitulo("correcao de divergencias de planta");
		 * 
		 * modelo.setCatalogoDescricao("19,20,30"); modelo.setCultura(cultura);
		 * modelo.setDataModificacao(data); modelo.setDescricao(descricao);
		 * modelo.setIlustracoes(ilustracao);
		 * modelo.setInstrucao("ftp://jbudb.wnwb.sq"); modelo.setNome("First 10");
		 * modelo.setProtuto(produto);
		 * 
		 * 
		 * 
		 * ProdutoRepositorio.save(produto); FotosRepositorio.save(fotos);
		 ...
		 * CulturaRepositorio.save(cultura); DescricaoRepositorio.save(descricao);
		 * DocumentosRepositorio.save(documentos);
		 * IlustracaoRepositorio.save(ilustracao); ModeloRepositorio.save(modelo);
		 * 
		 */

		/*
		 * // listar for (Produto objeto : ProdutoRepositorio.findAll()) {
		 * System.out.println("\n" + "Produto ID: " + objeto.getProdutoId() + "\n" +
		 * "Nome: " + objeto.getNome() + "\n" + "Numero do Produto  :" +
		 * objeto.getNumeroProduto() + "\n" + "Cor Produto : " + objeto.getCor() + "\n"
		 ...
		 * objeto.getUnidadeMedidaTamanho() + "\n" + "Unidade Medida Peso : " +
		 * objeto.getUnidadeMedidaPeso() + "\n" + "Peso Produto : " +
		 * objeto.getPesoProduto() + "\n" + "Dias Para Fabricar Produto : " +
		 * objeto.getDiasParaFabricarProduto() + "\n" + "Linha de Seguimento : " +
		 * objeto.getLinhaDoProduto() + "\n" + "Classe do Produto : " +
